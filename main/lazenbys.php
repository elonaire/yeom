...
<div id="login"><?php echo '<form method="POST" action="'.userLogin1($dbc).'"><input type="text" placeholder="User ID" name="uid"><br><input type="password" placeholder="password" name="pwd"><br><input type="submit" class="btn-success" value="login" name="login1"></form>'; ?>
	...
	<div class="row">
		<div class="col-4">
			<?php
				$sql = "SELECT * FROM time_tbl WHERE name='LAZENBYS'";
				...
				<div class="col-6">
					...
					<?php
						...
						while ($row = $result->fetch_assoc()) {
							echo "<li class='list-item'>".$row['name']."............................$".$row['price']."<form method='POST' action='".Food($dbc)."'><input type='hidden' name='fid' value='".$row['fid']."'><input type='hidden' name='sid' value='".$sess."'><input type='hidden' name='bal' value='".$bal."'><button type='submit' class='comment' name='commFood'>Comment</button><button type='submit' class='buy' name='buyFood1'>Buy</button></form></li>";
						}
						echo "</ul>";
					?>
				</div>
